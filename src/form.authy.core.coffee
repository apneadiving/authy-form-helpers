...
getByClassName = (name)->    document.getElementsByClassName(name)
createElement  = (tagName)-> document.createElement(tagName)
appendChild    = (child)->   document.body.appendChild child
addOnClickEvent = (callback)-> document.onclick = callback
destroyElement  = (element)->  element.parentNode.removeChild(element)
...
window.Authy.Core = do ->

  phone = (text)->
    text != "" and text.match(/^([0-9][0-9][0-9])\W*([0-9][0-9]{2})\W*([0-9]{0,5})$/)

  token = (text)->
    text != "" and text.match(/^\d+$/)

  getMatchingCountryItems = (text, callback)->
    text = text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") # Escape regular expression special characters
    reg  = new RegExp("^" + text, "i")
    for countryItem in countriesList
...
Tooltip = (options)->
  ...
  defaultOptions = ->
    ...
    options.triggerCloseId ?= "authy-tooltip-close"
  ...
  setupEvents = ->
    ...
    getById(options.triggerCloseId).onclick = ->
...
DropDown = (dropdownOptions)->
  ...
  getCountriesArr = ->
    self.countriesDropdown.getElementsByTagName("li")

  # Process the down-arrow key
  processKey40 = ->
    countriesArr = getCountriesArr()

    for li, i in countriesArr #search all the array which is active and set the next one as active
  ...
  #
  setupEvents = ->
    self.countriesInput.onblur = (event) ->
      processKey13()

    self.countriesInput.onfocus = ->
      setupCountriesDropdownPosition()
      self.countriesDropdown.style.display = "block"
      return

    self.countriesInput.onkeyup = (event)->
      self.countriesDropdown.style.display = "block"
      ...
      switch keyID
        when 13 # enter key
          ...
          return false
        when 40 # down key
          return false  if processKey40() == false
        when 38 # up key
          return false  if processKey38() == false
      self.search()

    self.countriesInput.onkeypress = (event) ->
      if getKeyCode(event) == 13
        processKey13() # Enter key
        return false

    self.countriesDropdown.onclick = (e)->
    ...
    self.countriesInput.onclick = (e)->
      if e && e.stopPropagation
        e.stopPropagation()
        self.countriesInput.focus()
        self.countriesInput.select()
      else
        e = window.event
        e.cancelBubble = true
  ...
  findAndSetupCountries = ->
    return unless self.countriesSelect
    countries = []
    i = 0
    ...
    for countryItem in countriesList()
      classActive = (if (activeItem is countryItem) then "active" else "")
    ...
    unless self.countryCodeInput.parentNode
  ...
  #
  # Calculates the country dropdown position based on the self.countriesInput
  #
  setupCountriesDropdownPosition = ->
    ...
    if width < 220
      width = 220
    self.countriesDropdown.setAttribute "style", "width: " + (width - 5) + "px; top: " + (pos[0] + 2 + self.countriesInput.offsetHeight) + "px; left: " + (pos[1] - 2) + "px;"

  countriesList = ->
    if dropdownOptions.defaultCountries
      Authy.Core.countriesList
    else
  ...
  #
  buildItem = (classActive, countryItem) ->
  ...
  #
  setCountryField = ->
    return unless self.countryCodeInput
    countryCode = self.countryCodeInput.value

    if countryCode isnt ''
      for countryItem in countriesList()
        if countryItem.code is countryCode
  ...
  do ->
    ...
    self.onSelect          = dropdownOptions.onSelect || ->
...
DropDown.prototype.destroy = ->
  destroyElement @countriesInput
  destroyElement @countryCodeInput
  destroyElement @countriesSelect
  destroyElement @countriesDropDown

DropDown.prototype.setCountryCode = (value)->
...
DropDown.prototype.search = ->
  ...
  Authy.Core.getMatchingCountryItems(@countriesInput.value, (countryItem)=>
    countriesAutocompleteList.appendChild(@buildItem(classActive, countryItem))
    classActive = ""
    matches = true
  ...
  if matches
    ...
    @setCountryCode(firstCountryCodeFound)

DropDown.prototype.autocomplete = (obj, hideList) ->
...
DropDown.prototype.hideAutocompleteList = ->
  if @countriesDropdown
    return @countriesDropdown.style.display = "none"
